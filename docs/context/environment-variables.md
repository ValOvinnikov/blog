...
| `DATABASE_URL`                                                 | web + db (runtime)                                 | required once `@blog/db` is actually consumed; pooled Neon connection string (`drizzle-orm/neon-http`), validated by `packages/db/src/utils/env/env.ts` — see `SPEC.md` §4/§8                                                                           |
