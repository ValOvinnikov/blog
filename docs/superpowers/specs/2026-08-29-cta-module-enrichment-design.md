...
**D16 ✔, 2026-08-29 — `content`'s inline `link` annotations resolve both internal and external references, not external-only.** Caught in review: the first implementation projected `content[]` raw with no deref of its `markDefs`, so an editor picking "Internal document" for an inline link inside CTA `content` got it silently downgraded to plain text — the exact silent-drop failure mode #1861 (§8.1) was closing, reproduced in a different field. Rejected as a shipped limitation. `packages/service`'s CTA query resolves `content[].markDefs[]` the same way `linkFragment` already resolves `link.internalReference` for actions (`blog_post`/`blog_topic`/`page_generic` deref, per the existing `selectByType` pattern) — there is no prior precedent for deref-ing inside a Portable Text array in this codebase (`module_content`'s `richText` has no link annotation at all), so this is new groqd territory, not a copy-paste.
...
- **D16 ✔, 2026-08-29** — **`content`'s inline links resolve internal AND external references.** Caught in review: the first implementation only resolved `url`-based external links, silently dropping internal-document links to plain text — reproducing #1861's failure mode in a different field. Rejected. `packages/service` derefs `content[].markDefs[]` the same way `linkFragment` already derefs actions' `internalReference`; no prior precedent in this codebase for deref inside a Portable Text array (`module_content`'s `richText` has no link annotation to begin with). §6.
