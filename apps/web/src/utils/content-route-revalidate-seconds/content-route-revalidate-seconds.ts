 ...
 * indefinitely. Well above the Data Cache's 3600s `isr()` TTL, but short
 * enough to bound a missed purge to part of a working day rather than a
 * full one — this is a correctness floor for a rare failure mode, not a
 * freshness target, so it trades off against regenerating every
 * tenant×locale path more often than that.
 */
