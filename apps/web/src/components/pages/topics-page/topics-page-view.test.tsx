...
const setup = customRender(TopicsPageView, makeTopicsPageView());

describe(TopicsPageView, () => {
  ...
  it('renders no supporting text paragraph when none is given', () => {
    ...
    expect(
      screen.queryByText('Browse every post by topic.'),
    ).not.toBeInTheDocument();
  });

  it('renders the Home › Topics breadcrumbs trail', () => {
    setup();
  ...
  it('renders the real composed taxonomy list markup — cards linking to each topic', () => {
    setup();

    const main = screen.getByRole('main');

    expect(
      within(main).getByRole('link', { name: /Engineering/ }),
