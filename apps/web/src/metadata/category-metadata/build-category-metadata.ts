...
/**
 * Metadata for a `/category/[slug]` page (unpaginated, `pageNumber`
 * omitted) or a `/category/[slug]/page/[page]` page (`pageNumber` ≥ 2).
 * `TCategory` carries no dedicated `seo`/OG fields (unlike
 * `TPostDetail.seo`), so this builds `Metadata` directly from the category's
 * own `title`/`description` rather than inventing fields the service layer
 * doesn't provide. Every page self-canonicalizes — page 2+ must never
 * canonical to `/category/[slug]`.
 *
