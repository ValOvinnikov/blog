...
import { getTopicsIndexPage } from './get-topics-index-page';
...
describe(getTopicsIndexPage, () => {
  ...
  it('resolves the tenant Sanity context, then forwards it to service.pages.topicIndex.v1.getIndexPage', async () => {
    ...
    await getTopicsIndexPage('tenant-1');
  ...
  it('returns the raw TResult from getIndexPage unchanged', async () => {
    ...
    await expect(getTopicsIndexPage('tenant-1')).resolves.toBe(result);
  });
});

describe('getTopicsIndexPage memoization', () => {
  afterEach(() => {
    vi.doUnmock('react');
    vi.resetModules();
  });
  ...
  it('dedupes the index-page query when called more than once in the same render pass with the same arguments', async () => {
    ...
    const { getTopicsIndexPage: freshGetTopicsIndexPage } =
      await import('./get-topics-index-page');

    await freshGetTopicsIndexPage('tenant-1');
    await freshGetTopicsIndexPage('tenant-1');
