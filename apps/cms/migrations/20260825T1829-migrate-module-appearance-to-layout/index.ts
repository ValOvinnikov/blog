 ...
 * onto the current `layout`/`heroLayout` field; field mapping lives in
 * `./transform.ts`. Idempotency guard skips a doc whose `layout` is already
 * defined — the *target* shape, not the presence of `appearance` — so a doc
 * that transiently carries both is never re-wrapped or clobbered.
 */
