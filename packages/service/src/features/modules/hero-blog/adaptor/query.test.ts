...
describe('heroBlogModuleQuery', () => {
  ...
  it('falls back to the newest published featured post when postSource is not PINNED', () => {
    expect(heroBlogModuleQuery.query).toContain(
      '*[_type == "page_post"][featured == true][publishedAt <= now() && defined(headingBlock.heading) && defined(author) && defined(topic) && defined(content) && defined(seo)] | order(publishedAt desc)[0]',
    );
  });
