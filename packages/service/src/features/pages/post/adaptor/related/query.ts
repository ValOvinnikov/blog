...
import { RELATED_POSTS_CANDIDATE_LIMIT } from './constants';
 ...
 * in the query itself — this fetches the `RELATED_POSTS_CANDIDATE_LIMIT`
 ...
 * `RELATED_POSTS_CANDIDATE_LIMIT` is a deliberate cap: large enough that a
 * genuinely closer (higher shared-tag-count) match published slightly
 ...
 * `relatedByCategoryQuery`'s `.slice(0, 6)` bound below).
 */
export const relatedByTagsQuery = q
  ...
  .slice(0, RELATED_POSTS_CANDIDATE_LIMIT)
  .project((sub) => ({
    ...postCardFragment,
    tagIds: sub
      .field('tags[]')
