...
describe('fold-page-topic-index-into-modules migration', () => {
  it('is scoped to page_topicIndex', () => {
    expect(migration.documentTypes).toEqual(['page_topicIndex']);
  });

  it('delegates document() to migrateTopicIndexPage', () => {
    const doc = {
      ...baseDoc,
      taxonomyList: { _ref: 'list-1' },
    };

    expect(migration.migrate.document(doc)).toEqual(migrateTopicIndexPage(doc));
  });
});
