...
/**
 * The string a destination collapses to for dedup — two legacy links with
 * the same key resolve to one created `link` document. Keyed on the
 * destination itself (the referenced document, or the URL), never on
 * `label` — two links can share a label and point elsewhere.
 */
