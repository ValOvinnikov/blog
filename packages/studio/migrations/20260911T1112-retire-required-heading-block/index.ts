 ...
 * the type:
 *
 *   - `page_post`: rewrites `headingBlock._type` from `requiredHeadingBlock`
 *     to `headingBlock` — the field itself is untouched, only the nested
 *     type discriminator changes.
 ...
 *
 * Idempotency guards, independent per document type:
 *   - `page_post`: only rewrites when `headingBlock._type` is exactly
 *     `requiredHeadingBlock` — a document already on `headingBlock`, or with
 *     no `headingBlock` at all, is left alone.
...
export const renamePagePostHeadingBlockType = (doc: TPagePostDoc) => {
...
const DOCUMENT_TYPES = ['page_post', 'module_heroStatement'];
...
export default defineMigration({
  ...
  migrate: {
    document(doc) {
      ...
      return renamePagePostHeadingBlockType(doc as unknown as TPagePostDoc);
    },
  },
});
