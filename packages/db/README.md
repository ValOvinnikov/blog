...
| `db:provision-tenant`   | `pnpm --filter @blog/db db:provision-tenant -- --tenant-id=...`                    | Runs `scripts/provision-tenant/run.ts` — the six-step tenant provisioning workflow (Sanity project, seed content, Studio deploy, token persistence, domain mapping, revalidate webhook). Invoked only by the `provision-tenant.yml` CI workflow. |
