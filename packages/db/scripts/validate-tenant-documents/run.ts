...
async function resolveOpenFindingIfAny(tenant: TTenant): Promise<void> {
  ...
  if (!result.ok) {
    console.error(
      `validate-tenant-documents: resolveFinding failed for tenant "${tenant.id}" (domain "${tenant.primaryDomain}"): ${result.error}`,
    );
  }
}
...
async function validateOne(
  ...
  if (
    ...
    summary.skipped += 1;
    console.log(
      `validate-tenant-documents: skipping tenant "${tenant.id}" (domain "${tenant.primaryDomain}") — status="${credentials.status}" deprovisionedAt=${credentials.deprovisionedAt ? credentials.deprovisionedAt.toISOString() : 'null'}`,
    );
    return;
  }
  ...
  try {
    ...
    summary.errors += 1;
    console.error(
      `validate-tenant-documents: sanity documents validate failed for tenant "${tenant.id}" (domain "${tenant.primaryDomain}"): ${sanitizeLogMessage(error)}`,
    );
    return;
  }
  ...
  if (!openResult.ok) {
    summary.errors += 1;
    console.error(
      `validate-tenant-documents: openFinding failed for tenant "${tenant.id}" (domain "${tenant.primaryDomain}"): ${openResult.error}`,
    );
    return;
  }
