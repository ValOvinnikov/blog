...
export async function notifyOperatorsOfDocumentValidationFailure({
  ...
  if (!resendApiKey) {
    console.log(
      `validate-tenant-documents: RESEND_API_KEY is unset — skipping operator notification for tenant "${tenant.id}".`,
    );
    return;
  }
  ...
  try {
    ...
    if (recipients.length === 0) {
      console.log(
        `validate-tenant-documents: no SUPERADMIN admins on file — skipping operator notification for tenant "${tenant.id}".`,
      );
      return;
    }
    ...
    console.error(
      `validate-tenant-documents: failed to notify operators for tenant "${tenant.id}": ${sanitizeLogMessage(error)}`,
    );
  }
}
