...
export async function notifyOperatorsOfDocumentValidationFailure({
  ...
  if (!resendApiKey) {
    console.log(
      `validate-tenant-documents: RESEND_API_KEY is unset — skipping operator notification for tenant "${tenant.id}" (domain "${tenant.primaryDomain}").`,
    );
    return;
  }
  ...
  try {
    ...
    if (recipients.length === 0) {
      console.log(
        `validate-tenant-documents: no SUPERADMIN admins on file — skipping operator notification for tenant "${tenant.id}" (domain "${tenant.primaryDomain}").`,
      );
      return;
    }
    ...
    const { error } = await resend.emails.send({
      ...
      to: recipients,
      subject: `Tenant "${tenant.name}" (${tenant.primaryDomain}) has invalid Sanity documents`,
      html: `<p>Tenant <strong>${escapeHtml(tenant.name)}</strong> (domain <code>${escapeHtml(tenant.primaryDomain)}</code>, id <code>${escapeHtml(tenant.id)}</code>) — ${severityCopy}. ${invalidDocumentCount} document(s) failed validation.</p><p>See the tenant's detail page in the platform admin panel for the full list.</p>`,
    });
    ...
    console.error(
      `validate-tenant-documents: failed to notify operators for tenant "${tenant.id}" (domain "${tenant.primaryDomain}"): ${sanitizeLogMessage(error)}`,
    );
  }
}
