...
describe('0008_silly_xorn (bookmarks/subscribers tenant_id backfill)', () => {
  it('backfills pre-existing bookmark and subscriber rows to the sole existing tenant', async () => {
  ...
  it('still applies cleanly against empty bookmarks and subscribers tables', async () => {
    const client = new PGlite();
    const db = drizzle(client, { schema });
