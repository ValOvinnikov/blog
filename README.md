...
| **Lighthouse CI** (`lighthouse.yml`)                | every PR to `main`                                      | Advisory Lighthouse CI budget assertions (performance ≥ 90, accessibility ≥ 95, best-practices ≥ 90, SEO ≥ 95 — `.lighthouse/budgets.json`, documented in `.lighthouse/README.md`) against **production** (`https://valstack.dev`) for `/` and one post page — not a per-PR preview (none exists yet, SPEC §13); `development` was tried first (#399) but is unsuitable (deliberately `noindex` per #841, plus cold-start noise), so this checks production's current health on every PR rather than the PR's own diff, until #275 lands a true preview target (#826). Guarded on the repo Variable `LIGHTHOUSE_URLS`; no-ops green if unset.             |
...
than the PR's own diff (no per-PR preview target exists yet, #275), which
makes a PR-blocking failure there potentially unrelated to that PR's changes.
