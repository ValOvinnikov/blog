...
| **Lighthouse CI** (`lighthouse.yml`)                | every PR to `main`                                      | Advisory Lighthouse CI budget assertions (performance ≥ 90, accessibility ≥ 95, best-practices ≥ 90, SEO ≥ 95 — `.lighthouse/budgets.json`, documented in `.lighthouse/README.md`) against **production** (`https://valstack.dev`) for `/` and one post page — not a per-PR preview (none exists or is currently tracked, SPEC §13); `development` was tried first (#399) but is unsuitable (deliberately `noindex` per #841, plus cold-start noise), so this checks production's current health on every PR rather than the PR's own diff, until a true per-PR preview target exists (#826). Guarded on the repo Variable `LIGHTHOUSE_URLS`; no-ops green if unset. |
| **Playwright smoke** (`playwright-smoke.yml`)       | every PR to `main`                                      | Minimal e2e smoke suite (`apps/web/e2e/`): home page and one post-detail page (discovered dynamically from a live link, no hardcoded slug) each render 200 with zero console errors, run against `SMOKE_URL`. **Currently a no-op green** for the same reason as Lighthouse CI above — no PR preview deploys yet, and no per-PR preview mechanism is currently tracked (#275 closed via #822 without delivering one), so the job is guarded on the repo Variable `SMOKE_URL`.                                                                                                                                                                                        |
...
([#450](https://github.com/ValOvinnikov/blog/pull/450)). Playwright smoke is
still a structural no-op — it depends on a preview/smoke URL this repo doesn't
produce yet (no PR preview deploys, per SPEC §13) — so promoting it stays a
deliberate follow-up once `SMOKE_URL` is configured and it can genuinely fail.
Lighthouse CI now targets production (#826) and **can** genuinely fail — it
stays advisory anyway because it checks production's current health rather
than the PR's own diff (no per-PR preview target exists or is currently
tracked), which makes a PR-blocking failure there potentially unrelated to
that PR's changes.
