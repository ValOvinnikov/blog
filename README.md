...
| **Zizmor** (`zizmor.yml`)                         | every PR to `main`                                      | Static security analysis of the workflow files themselves. Advisory today; proposed for promotion to a required check, pending the ruleset change — see [#462](https://github.com/ValOvinnikov/blog/issues/462).                                                                                                                                                                                                                                                                                                                           |
| **Actionlint** (`actionlint.yml`)                 | every PR to `main`                                      | Validates the workflow files' syntax, `${{ }}` expressions and `needs` graph, and shellchecks their `run:` blocks — the correctness half that Zizmor's security analysis does not cover. Runs the official binary, pinned by version + sha256 (no third-party action, no curl-to-shell). Advisory today; proposed for promotion to a required check, pending the ruleset change — see [#462](https://github.com/ValOvinnikov/blog/issues/462).                                                                                             |
| **Knip** (`knip.yml`)                             | every PR to `main`                                      | Reports unused files, exports, and dependencies (config in `knip.json`); read-only, fails on any finding or a stale ignore rule. Advisory today; proposed for promotion to a required check once it's had two weeks of zero false positives, pending the ruleset change — see [#462](https://github.com/ValOvinnikov/blog/issues/462).                                                                                                                                                                                                     |
| **Commitlint** (`commitlint.yml`)                 | every PR to `main`                                      | Backstop for the `.husky/commit-msg` hook: lints every commit in the PR's range against the root `commitlint.config.mjs` (Conventional Commits + this repo's `tooling` type). Merge commits are skipped; Dependabot's `chore(deps): …` messages are not separately exempted — they pass because they're already conventional. Advisory today; proposed for promotion to a required check, pending the ruleset change — see [#462](https://github.com/ValOvinnikov/blog/issues/462).                                                        |
| **Hooks** (`hooks.yml`)                           | every PR to `main`                                      | Shellchecks `.claude/hooks/*.sh` (outside actionlint's `run:`-block-only coverage) and runs `read-only-agent-guard.test.sh`'s, `gate-bypass-guard.test.sh`'s, `pre-bash-worktree-install-guard.test.sh`'s, and `test-writer-scope-guard.test.sh`'s deny/allow matrices against their respective guards. Advisory today (job name `Shellcheck + guard tests`); proposed for promotion to a required check, pending the ruleset change — see [#462](https://github.com/ValOvinnikov/blog/issues/462).                                        |
...
| **Claude Code** (`claude.yml`)                    | `@claude` mentions (owner-only, owner-authored threads) | Interactive agent runs on issues/PRs. Advisory today; whether it should become a required check is an undecided, open question — see [#462](https://github.com/ValOvinnikov/blog/issues/462), which does not propose changing this one.                                                                                                                                                                                                                                                                                                    |
...
| **Lighthouse CI** (`lighthouse.yml`)              | every PR to `main`                                      | Advisory Lighthouse CI budget assertions (performance ≥ 90, accessibility ≥ 95, best-practices ≥ 90, SEO ≥ 95 — `.lighthouse/budgets.json`, documented in `.lighthouse/README.md`) against a preview URL for `/` and one post page. **Currently a no-op green**: this repo has no PR preview deploys for web by design (SPEC §13), so the job's Lighthouse step is guarded on the repo Variable `LIGHTHOUSE_URLS` and only runs once [#275](https://github.com/ValOvinnikov/blog/issues/275) resolves how a preview/smoke URL is produced. |

**Required status checks on `main`:** Type-check, Lint, Test, Typegen, Build,
Migrations, and Dependency Review. Everything else — including Zizmor,
Actionlint, Knip, Commitlint, and Hooks, all proposed for promotion in
[#462](https://github.com/ValOvinnikov/blog/issues/462) — is advisory today:
a red result there does not currently block a merge. CI (and the required
...
**Lighthouse CI stays advisory by design** ([#450](https://github.com/ValOvinnikov/blog/pull/450))
— its budgets depend on a preview/smoke URL this repo doesn't produce yet
([#275](https://github.com/ValOvinnikov/blog/issues/275)), so it isn't a
required check.
...
predates #462 and #462 does not reopen it — only `claude.yml`'s required/
advisory status is a genuinely open question, flagged but not decided in
[#462](https://github.com/ValOvinnikov/blog/issues/462).
