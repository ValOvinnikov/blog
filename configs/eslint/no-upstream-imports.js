...
// apply everywhere it's spread in (currently config.js and utils.js). The
// `@blog/db` → `@blog/auth` layering (db must never import auth; auth
// legitimately imports db, so the ban is one-directional, not mutual) is
// package-specific and enforced directly in db.js instead.
