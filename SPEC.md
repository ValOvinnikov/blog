...
| `@blog/insight` | —                                                                  | Structured logger core — `createLogger` (single-line JSON via `console.*`), `LOG_LEVEL`, and its own copy of `sanitizeLogMessage` (temporary, deliberate duplication of `@blog/utils`'s copy — see that row's note). Consumed by both apps: `apps/web` and `apps/admin` each expose one shared logger at `src/utils/logger/logger.ts`, built with `createLogger` and its own `service` value, and import that rather than calling `createLogger` per module — the `service` field is what separates the two apps' lines downstream. `service`/`db`/`auth` never log; failures reach the caller, and the app layer logs them. | import React, Next.js, or any Sanity SDK; depend on any sibling                                                                      |
