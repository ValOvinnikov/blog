...
packages/
  ...
  config     Cross-cutting constants, generated Sanity types,         (@blog/config)
             ...
             the WCAG AA accent-hue contrast guard
  ...
  email      Shared HTML email shell, escapeHtml, Resend transport    (@blog/email)
configs/
  eslint, next, prettier, tailwind, tsconfig, vitest                  (@blog/*-config)
```

| Layer            | Imports                                                                                                             | Exposes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Must never                                                                                                                                                                                                                                                               |
| ---------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@blog/config`   | `utils`, `@portabletext/types` (types only)                                                                         | Constants (UPPERCASE key/value pairs) — cross-cutting ones only; a storage layer's own vocabulary lives with that layer, see `@blog/db` — the `routes` URL builder (single source of URL truth) and `pageHref`, the page-type → path resolver both `service`'s link transformer and the Studio's action previews use, generated Sanity types + extracted schema, shared TS types, `/react` subpath for polymorphic prop helpers, and (alongside `PRESET_REGISTRY`) the WCAG AA `isAccentHueAccessible` accent-hue contrast guard and `resolveTenantEmailBrand`, which runs the same OKLCH recipes `apps/web` renders as CSS to return literal hex for `@blog/email`'s tenant shell. Also `TPortableTextBlock` — the one resolved Portable Text block type, built on `@portabletext/types`' `PortableTextBlock` with an `IPortableTextLinkMark` (`_type: 'linkRef'`) mark, intersected with `{ _type: 'block'; _key: string }` (both pins load-bearing: without the first, an `Extract<…, { _type: 'block' }>` collapses to `never`; without the second it disagrees with typegen) — imported by both `service` and `web`. `@blog/db`'s `TEmailTemplateBlock` and `@blog/email`'s `TEmailBlock` are email-specific shapes, deliberately named apart from it. Style and list narrowing is the schema's job (`inlineText`, `proseText`, `richText`), not the type's | contain app logic; force React on non-React consumers (subpath!)                                                                                                                                                                                                         |
...
| `@blog/email`    | `utils`                                                                                                             | Shared HTML email infrastructure: two shells over one internal table-based layout — `buildOperatorShell` (takes **no** brand and renders the platform's own `PLATFORM_EMAIL_BRAND`, sourced from `configs/tailwind/theme.css`'s light-mode tokens via `@blog/utils`'s `oklchToHex`) and `buildTenantShell` (**requires** a resolved `brand`, produced by `@blog/config`'s `resolveTenantEmailBrand`). The split is the enforcement: an operator alert cannot render in a tenant's colours because there is no argument to pass, and a tenant email cannot fall back to the platform look because omitting the brand does not compile. Inlined styles, no external stylesheet. Also the canonical `escapeHtml`, and a `sendEmail` transport wrapping Resend (reads `RESEND_API_KEY`) — the only function in the package that does I/O. Its upstream is `utils` only, which is what lets `apps/web`, `apps/platform`, and `@blog/auth` all depend on it without a cycle — `@blog/auth` sits above `db`, so a dependency on either would close one.                                                                                                                                                                                                                                                                                                                 | import React, `@blog/ui`, `@blog/service`, `@blog/db`, `@blog/auth`, `@blog/studio`, or `@blog/insight`; log — failures return to the caller                                                                                                                             |
| `@blog/service`  | `config`, `utils`, Sanity SDKs                                                                                      | The versioned `service` facade (`service.pages.post.v1.getPost(slug)` …), view-model types (`TPostDetail`, `THomePage`, …), the image URL builders (`urlForImage`, `urlForSanityImage`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | import React or `@blog/ui`; return raw Sanity docs; emit rendered image URL strings; fake defaults                                                                                                                                                                       |
...
as an `ISanityImage` view-model (asset id, `alt`, hotspot, crop, LQIP,
dimensions) and never returns a rendered URL string. `apps/web` builds the URL
at the edge, where the request's tenant is already resolved — through
`urlForSanityImage(image, tenant, options?)` for a concrete URL (OG and Twitter
images, JSON-LD, the favicon, author avatars), and through
`getSanityImageBaseUrl(tenant)`, resolved once per request by
`SanityImageBaseUrlProvider`, for the `SanityImage` bridge's own responsive
srcset. The Sanity SDKs stay inside `@blog/service`; `apps/web` calls those two
exported helpers rather than importing `@sanity/image-url` itself. Transform
options belong at the call site that knows the rendered size, so a component
asking for a 64px avatar states that itself instead of receiving whatever crop
a transformer happened to bake in.
...
`PortableText` the CTA module uses — `bio` is `proseText`, which
...
changes that decision. That #2477 reclassification was benign: reading
`headers()` in an otherwise-fully-static route just moved it to `ƒ` at build
time. The identical kind of read was fatal, not benign, when
`app/not-found.tsx` made it before #3191 — at that point it ran as the
fallback for `[tenant]/[locale]`'s on-demand blocking generation, a route
Next had already committed to a static/ISR shape at build time, and
