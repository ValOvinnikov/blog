...
| `@blog/utils`   | `culori` (OKLCH color math)                                        | Pure helpers (`safeAsync`, primitives, `oklchToHex`), `sanitizeLogMessage` (log-injection sanitizer — temporarily duplicated into `@blog/insight` rather than shared, removed from here once every call site has migrated off it)                                                                                                                                                                                                                                                                               | depend on any sibling                                                                                                                                                                                           |
| `@blog/insight` | —                                                                  | Structured logger core — `createLogger` (single-line JSON via `console.*`), `LOG_LEVEL`, and its own copy of `sanitizeLogMessage` (temporary, deliberate duplication of `@blog/utils`'s copy — see that row's note). Consumed by both apps: `apps/web` and `apps/admin` each expose one shared logger at `src/utils/logger/logger.ts` (`createLogger({ service: 'web'                                                                                                                                           | 'admin' })`) and import it rather than calling `createLogger`per module — the`service`field is what separates the two apps' lines downstream.`service`/`db`/`auth` never log; they return errors to the caller. | import React, Next.js, or any Sanity SDK; depend on any sibling |
