...
| `@blog/utils`   | `culori` (OKLCH color math)                                        | Pure helpers (`safeAsync`, primitives, `oklchToHex`), `sanitizeLogMessage` (log-injection sanitizer — temporarily duplicated into `@blog/insight` rather than shared, removed from here once #1640/#1641 migrate every call site off it — see #1642)                                                                                                                                                                                                                                                            | depend on any sibling                                                                                                                |
| `@blog/insight` | —                                                                  | Structured logger core — `createLogger` (single-line JSON via `console.*`), `LOG_LEVEL`, and its own copy of `sanitizeLogMessage` (temporary, deliberate duplication of `@blog/utils`'s copy — see that row's note). Not yet consumed by any app; `web`/`admin` migrate their `console.*` call sites onto it in #1640/#1641.                                                                                                                                                                                    | import React, Next.js, or any Sanity SDK; depend on any sibling                                                                      |
