...
| `@blog/db`      | `config`, `utils`, Drizzle/Neon SDKs   | Typed query/mutation functions over Neon Postgres (Auth.js adapter tables, comments, ratings, bookmarks, subscribers, plus the `tenants`/`tenant_domains`/`memberships` registry and the global `admins` table) — the relational sibling to `service`, not a dependent of it | import React, `@blog/ui`, `@blog/service`, or any Sanity SDK; be imported by `cms`/`service`/`ui` |
